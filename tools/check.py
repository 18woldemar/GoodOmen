...
ENGINE = [
    ...
    ("the engine runs a level, driver and all",
     ["cargo", "run", "--quiet", "--release",
      ...
      "--expect-events", "13500", "--expect-survived", "12150"], None),
    ("proximity reaches the scripts: walking makes something play",
     ["cargo", "run", "--quiet", "--release",
      "--manifest-path", "engine/Cargo.toml", "--", "$MDK2_GOG",
      "--run", "6", "1", "40", "--expect-playing", "1",
      "--expect-moves", "1200"], None),
    ("the room graph culls what the engine draws",
     ["cargo", "run", "--quiet", "--release",
      "--manifest-path", "engine/Cargo.toml", "--", "$MDK2_GOG",
      "--play", "1", "1", "--expect-drawn", "12929"], None),
