...
| `.tex` | A mip chain at exactly **4 bits per pixel** down to 8×8, then the 4×4/2×2/1×1 levels as raw BGRA (the 84-byte tail) — unless the u32 at 0x24 is 0, which means one uncoded level of `w × h × channels` bytes and no chain at all. The 4 bpp coding is a **multi-mode block codec**: 8×4-pixel blocks in 16 bytes, each two independent 4×4 sub-blocks, with **six layouts** chosen by the top four bits. Fully decoded — see [`tools/texdec.py`](tools/texdec.py). |
...
| `.str` | 686 entries of `{id, text, sound}`. Text is a run of 16-bit units **each holding one code-page byte** — it reads as UTF-16 and is not one, which five Western language tables agreed on and the Russian one refuted; the code page arrives with the release's own font atlas. `sound` names the `.wav` that speaks the line. |
